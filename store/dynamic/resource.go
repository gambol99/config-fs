...
func NewDynamicResource(filename, content string, store kv.KVStore) (DynamicResource, error) {
	...
	if agent, err := discovery.NewDiscovery(config.serviceUpdateChannel); err != nil {
		return nil, err
	} else {
